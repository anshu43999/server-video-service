...
本部署覆盖 `server-video-service`、独立 `model-converter`、PostgreSQL 16 和 MediaMTX 1.20.1。支持基线为 CentOS Stream 9，以及与 RHEL 9 兼容的 Rocky Linux/AlmaLinux。CentOS Linux 7 已于 2024-06-30 结束维护，不作为生产部署目标；若现有服务器仍是 CentOS 7，应先升级操作系统。
...
sudo install -d -o root -g 10002 -m 0750 /opt/aiyolo/calibration
sudo cp -a /secure/calibration-delivery/. /opt/aiyolo/calibration/
sudo chown -R root:10002 /opt/aiyolo/calibration
sudo chmod -R u=rwX,g=rX,o= /opt/aiyolo/calibration
sudo restorecon -RFv /opt/aiyolo/calibration
...
- `CALIBRATION_MOUNT_PATH` 填宿主机校准集目录，转换容器以只读方式挂载到 `/calibration`。
...
后端容器固定使用 UID/GID `10001`，转换容器固定使用 UID/GID `10002`。上面的组权限允许容器读取模型和校准集，但不能修改宿主机交付文件；若企业基线要求使用 ACL，可使用 `setfacl -m u:10001:rX` 和 `setfacl -m u:10002:rX` 达到相同效果。
...
bash deploy/centos/deploy.sh logs model-converter
```
...
```bash
bash deploy/centos/deploy.sh backup-db
# 修改 deploy/centos/.env 中的 VIDEO_SERVICE_IMAGE 和 MODEL_CONVERTER_IMAGE
bash deploy/centos/deploy.sh config
bash deploy/centos/deploy.sh up
```
