...
对应任务：`M27-T01`。调用方是视频服务，提供方可以部署在本机、局域网 GPU/CPU 主机或独立转换服务器。远程 URL 是 HTTP API endpoint，不冒充 Python 路径。
...
配置文件、任务快照、日志与 API 只保存环境变量名，不保存或回显令牌值。endpoint 禁止 URL 内凭据、query 和 fragment。生产环境不得开启明文 HTTP。
