  ...
  async function refresh() {
    ...
    try {
      ...
      byId('conversion-jobs').innerHTML = jobs.length ? jobs.slice(0, 20).map(job => {
        ...
        return `<article class="conversion-job"><div class="conversion-job-top"><strong>${safe(actions[job.action])} · ${safe(metadata.name || '转换环境')}</strong><span class="conversion-job-status ${safe(job.status)}">${safe(labels[job.status])}</span></div><small>${safe(new Date(job.created_at * 1000).toLocaleString())} · 第 ${job.attempt} 次执行 · ${safe(jobTiming(job))}${calibration ? ' · ' + safe(calibration.name + ' ' + calibration.version) : ''}</small>${renderJobProgress(job)}${result.server_ready ? '<div class="conversion-state-pair"><span>服务端已验证</span><span>' + (result.android_ready || conversion?.status === 'succeeded' ? '移动端产物已验证' : '移动端单独处理') + '</span></div>' : ''}${text ? `<p class="${job.error ? 'conversion-error' : ''}">${safe(text)}</p>` : ''}<div class="conversion-actions">${['failed','interrupted'].includes(job.status) ? `<button class="ghost-btn" data-retry="${job.id}">按当前配置重试</button>` : ''}${canConvert ? `<button class="ghost-btn" data-convert="${metadata.upload_id}">生成移动端模型</button>` : ''}<button class="ghost-btn" data-log="${job.id}">查看日志</button></div></article>`;
