...
(() => {
  ...
  panel.innerHTML = `
    ...
    <div class="conversion-grid"><div>
      ...
      <form id="conversion-config-form">
        ...
        <label class="field">INT8 校准数据 YAML<input id="conversion-calibration" placeholder="COCO 测试可用 coco8.yaml"></label>
        <p>业务模型使用代表性校准数据；路径以转换环境中的文件系统为准。已上传模型的输入尺寸保持不变。提交环境检测或移动端转换前，系统会自动检测 WSL 连通性。</p>
  ...
  let connected = false, refreshing = false, uploading = false, jobs = [], lastJobs = '';
  ...
  async function readConfig() {
    ...
    byId('conversion-calibration').value = c.calibration_data;
  ...
  async function refresh() {
    ...
    try {
      ...
      byId('conversion-jobs').innerHTML = jobs.length ? jobs.slice(0, 20).map(job => {
        ...
        const conversion = jobs.find(other => other.action === 'mobile' && other.metadata.upload_id === job.metadata.upload_id);
        ...
        return `<article class="conversion-job"><div class="conversion-job-top"><strong>${safe(actions[job.action])} · ${safe(job.metadata.name || '转换环境')}</strong><span class="conversion-job-status ${safe(job.status)}">${safe(labels[job.status])}</span></div><small>${safe(new Date(job.created_at * 1000).toLocaleString())} · 第 ${job.attempt} 次执行</small>${result.server_ready ? '<div class="conversion-state-pair"><span>服务端已验证</span><span>' + (result.android_ready || conversion?.status === 'succeeded' ? '移动端产物已验证' : '移动端单独处理') + '</span></div>' : ''}${text ? `<p class="${job.error ? 'conversion-error' : ''}">${safe(text)}</p>` : ''}<div class="conversion-actions">${['failed','interrupted'].includes(job.status) ? `<button class="ghost-btn" data-retry="${job.id}">按当前配置重试</button>` : ''}${canConvert ? `<button class="ghost-btn" data-convert="${job.metadata.upload_id}">生成移动端模型</button>` : ''}<button class="ghost-btn" data-log="${job.id}">查看日志</button></div></article>`;
  ...
  byId('conversion-config-form').addEventListener('submit', async event => {
    ...
    const config = {mode:byId('conversion-mode').value, distribution:byId('conversion-distribution').value,
      ...
      calibration_data:byId('conversion-calibration').value, timeout_seconds:Number(byId('conversion-timeout').value), auto_convert:byId('conversion-auto').checked,
  ...
  byId('conversion-upload-form').addEventListener('submit', async event => {
    ...
    xhr.upload.onprogress = event => { if (event.lengthComputable) byId('conversion-progress').value = Math.round(event.loaded / event.total * 100); };
    xhr.onload = async () => {
      if (xhr.status >= 200 && xhr.status < 300) { message('上传完成，正在排队验证 PT。验证通过后可在下方按流绑定。'); await refresh(); }
    ...
    byId('conversion-progress').value = 0; xhr.send(file);
  });
  byId('conversion-jobs').addEventListener('click', async event => {
    ...
    try {
      ...
      if (button.dataset.convert) await api(`/api/conversion/uploads/${button.dataset.convert}/mobile`, {method:'POST'});
